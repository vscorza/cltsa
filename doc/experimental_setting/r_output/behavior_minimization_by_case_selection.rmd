...
## Compared times between plant vs minimization size

```{r echo=FALSE}
#dividing compared data by case
cases <- c("Ahb","Genbuf","Lift","Collector", "Exploration","Airport","Bidding","Transfer","Travel")
cases_names <- c("AHB Arbiter","GenBuf","Lift Controller","Collector", "Exploration Robot","Airport Tower","Bidding Workflow","Transfer Line","Travel Agency" )

for (i in 1:length(cases)){
  current_data <- subset(compared_data, grepl(cases[i], name))  

  print(ggplot(current_data, aes(x=plant_transitions, y=minimization_transitions_liveness)) +
  	labs(title=paste("Plant vs minimization size (liveness)", cases_names[i])) +
  ...
  print(ggplot(current_data, aes(x=plant_transitions, y=minimization_transitions_safety)) +
  	labs(title=paste("Plant vs minimization size (safety)", cases_names[i])) +
  	xlab(expression(paste("|",Delta["E"],"|"))) +
    ylab(expression(paste("|",Delta["E'"],"|"))) +
  	scale_y_log10() +
    scale_x_log10() +	
