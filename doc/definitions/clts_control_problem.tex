...
\begin{definition}
	...
	Given CLTS \cltsDefShareSigmaIdx{C} and \cltsDefShareSigmaIdx{E}, where $\Sigma$ is partitioned into actions controlled and monitored (non controllable) by $C$ ($\Sigma=\controlSet \; \uplus \;\nonControlSet$), we say that $C$ is a legal CLTS for $E$ if for all $(s_e,s_c) \in E \parallel_* C$ it holds that:
...
\begin{definition}
	...
	Let \cltsDefShareSigmaIdx{E} be bi-partite w.r.t. $\controlSet \subseteq \Sigma$, then \controlProblemDef constitutes a CLTS control problem. If a solution to $\controlProblem$ exists, such a solution will be a CLTS $M$, legal w.r.t. $E$ and $\controlSet$, such that, $E \parallel_* M \models \varphi$.
\end{definition}
