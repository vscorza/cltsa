\section{Two Behavior Minimization Procedures}\label{sect:solution}
...
%
%\begin{definition}\label{def:minimal-non realizability legal-plant}\emph{(Minimal Counter-Strategy Preserving Sub-CLTS)}
%Given a control problem $\mathcal{I'}=<E', \mathcal{C}, \varphi>$
%where $E' = (S_{E'}, \Sigma, \Delta_{E'}, s_0)$ we say that 
%$E'$ is a minimal Alternating Sub-CLTS for $E$
%if $I'$ is not realizable then the following holds:
%\[
%\begin{matrix}
%\forall E'' \text{s.t. } E'' \text{ is a closest Alternating Sub-CLTS for } E'\\
%\implies\\
...
%$E'$ should be a minimal Alternating Sub-CLTS for $E$.

%The minization algorithm described below yields
%one minimal $E'$ Alternating Sub-CLTS
%for $E$ and $\mathcal{C}$.
...
%the closest Alternating Sub-CLTS is computed from the resulting partial structure 
%and a one step back track is fired every time a realizable Alternating Sub-CLTS is found.
% A minimal Alternating Sub-CLTS is found when no further reduction is available.
...
%\input{Proof_CLTS_Preserves_Realizability}
...
\begin{lemma}\label{def:induction-preserves}(\emph{Alternating Sub-CLTSs preserve realizability})
