...
Automata-based approaches adopt define a communication model based on events (where only one 
...
used to repair the initial GR(1) specification. In ~\cite{maoz2021unrealizable} the authors introduce a conceptual framework for incremental core computation that lays the foundation for writing more effective minimization algorithms (e.g. \texttt{QuickCore}) while also providing a procedure that can compute all existing unrealizable cores (\texttt{Punch}).
