...
The number of transitions is used as the representation of absolute volume for two reasons, first, because it is a measure of how many options the user has when exploring the minimized plant the  and second, because it is the defining parameter in the complexity of the synthesis algorithms. 
...
As expected with any algorithm working with enumerative models, the minimization technique still under performs against symbolic approaches on both size and time. 

%time
In figure ~\ref{fig:size_vs_diag_time} we expose the relation between diagnosis time and plant size (measured in number of initial transitions). We run each instance seven times and reported the mean as both the time value for the table and the central dot, the error bars show the distance to the maximum and minimum time in each case. We argue that the performance of the algorithm is linearly consistent (the lineal fit having a p-value of $< 3.387e^{-11}$) across the range of sizes present in the test set (going from tens to hundred thousands of transitions).
We show the behavior of the technique while diagnosing the first non realizable version of the generic buffer for two senders, figure ~\ref{fig:genbuf_2_time_plot} shows the time taken at each step of the minimization and fig. ~\ref{fig:genbuf_2_size_plot} shows the incremental reduction of the plant as the minimization progresses. 
%controllability and post processing
We also found that the amount of minimization achievable seems to be related to how controllable the result expressed in figure  ~\ref{fig:min_ctr_vs_min_pct} as the number of controllable transitions in the minimization over the number of total transitions. The relation seems to fit a lineal model (p-value $< 2.2e^{-11}$).
%final remarks
