...
The extent to which the minimization technique serves its diagnostic purpose is defined by its soundness, efficiency and the information it provides to the engineer. 
Our guiding research questions are:
\begin{description}
	\item[RQ1] Can the technique give feedback for specifications found in the literature? 
...
Since to the best of our knowledge previous work was presented for reactive systems
is somewhat impossible to propose a direct comparison with existing techniques that are similar in spirit, since, for instance, in ~\cite{DBLP:conf/hvc/KonighoferHB10} the minimization is applied on the set of environmental safety formulas, which if evaluated under the behavioral perspective increments the volume of the plant represented as an automaton, and in ~\cite{DBLP:conf/sigsoft/KuventMR17} a simplification of the symbolic counter strategy is presented as a LTS composed of attractors and trap states, which describe a symbolic behavior and require that the initial specification is presented in formulaic form. Our technique adds a complimentary view to the unrealizability diagnosis problem.

Research question \textbf{RQ1} is answered by observing the relation between synthesis and diagnosis time of existing specifications. Research question \textbf{RQ2} is answered by comparing the reduction volume of our technique with respect to the closest approach (~\cite{DBLP:conf/hvc/KonighoferHB10}). 
...
In order to avoid bias, we used specifications taken from existing benchmarks and literature, prioritizing those where an unrealizable version was already present and mutating those where none were available in order to achieve unrealizability. 
...
These modifications are of two kinds, the first removes a required liveness assumptions from the original specification, the second relaxes a part of the environment by removing a safety environmental constraint. 
...
In both cases unrealizability is introduced by constraining the system or relaxing the environment beyond its intended capabilities. We restricted ourselves to removing only liveness assumptions under the premise that it is usually the case that the user has a clear intent of what the system should achieve (liveness guarantees) but needs to gain knowledge of the environment in which it will operate.  Because of this we are not considering the cases where unrealizability is caused by the fact that the guarantees are too strong. One variation per case is presented, in the parameterized examples, most of the liveness modifications produce symmetrical diagnosis. Our approach follows Cimatti's ~\cite{DBLP:conf/vmcai/CimattiRST08} in creating minimaly unfulfillable conflict by removing the smaller set of liveness assumptions to achieve non realizability.
 
Our benchmark is divided in two groups.The first group comes from the reactive synthesis domain, where behavior is described as a conjunction of safety formulae. Even though our technique assumes an event-based specification in which the behavior is represented as an automaton we decided to evaluate existing cases from reactive systems. The motivation for this is twofold, first we wanted to avoid bias when building a new benchmark by selecting only those cases that are more beneficial to the technique, and second unrealizable specifications have been already published and studied for this domain.
...
As we stated before we will take the minimization amount as a proxy how understandable the diagnosis is. Thus having a reduction comparable to that of our closest reference, and showing that it maintains some measure of consistency under scaling we assume that the technique should be helpful for the engineer.
At this point we propose that \textbf{RQ1} is answered affirmatively, since the reduction proportion is comparable to the closest related work w.r.t. to the original specification. We also observe that for most cases the time taken while diagnosing is somewhat comparable to what it takes to synthesize the realizable case. The average reduction presented in $v_{\mathcal{U}}$ answers \textbf{RQ2} quantitatively and shows that a large portion of the initial behavior can be usually removed.
