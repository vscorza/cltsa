...
/**
 * Returns the equivalent entry for the provided boolean array if one exists, if
 * none exists it adds it IMPORTANT: all arrays are removed as a they are
 * replaced by new entries, clone entry if persistent use is expected
...
bool *automaton_bool_array_hash_table_add_or_get_entry(
  ...
  if (copy_entry)
    return automaton_bool_array_hash_table_get_entry(table, entry);
  return entry;
}
/**
 * Returns the equivalent entry for the provided boolean array if one exists
 * IMPORTANT: all arrays are removed as a they are replaced by new entries,
 * clone entry if persistent use is expected
 * @param table the hash table
 * @param entry the boolean array to be searched
 * @return the boolean array already stored that is equivalent to the one
 * provided
...
/**
 * Adds a copy of the boolean array as an entry into the hash table following a
 * FIFO scheme
 * @param table the hash table
 ...
 * @param table the hash table
 * @param entry the entry to be evaluated, first BOOL_SIZE values will be
 * interpreted  as an int, same for the second part
  ...
  if (table->variable_count > BOOL_ARRAY_HASH_TABLE_SIZE) {
    ...
    for (i = 0; i < limit; i++) {
      *second_level_index <<= 1;
      if (entry[i + BOOL_ARRAY_HASH_TABLE_SIZE])
        *second_level_index |= 0x1;
    }
  }
}
