...
/**
 * Gets the union of alphabets in the composition and a map of labels in the
 * composition to global indexes
 ...
 * @param alphabet_count placeholder for the size of the composition local
 * alphabet
 * @param compose_to_global an array of uint that maps composition local entries
...
/**
 * Increments the indexes array as a counter, if the max value is reached the
 * returned value is true
 * @param idxs an array of the current indexes of the components' transitions to
 * be evaluated
 * @param idxs_sizes the outgoing degree for the current component's state plus
 * one for the case where the transition is not applied
 * @param idxs_skip the list of transitions to be skipped when incrementing (if
 * one transition shares alphabet with an automaton whose state has no
 * transition bringing that element skip it altogether)
...
bool automaton_automata_compose_increment_idxs(uint32_t *idxs, uint32_t *idxs_sizes, bool **idxs_skip, uint32_t automata_count) {
  ...
  do {
    ...
    set_anew = false;
    do {
      if (idxs[i] < (idxs_sizes[i] - 1)) { // increment current digit if
                                           // possible
        idxs[i]++;
#if DEBUG_COMPOSITION
        printf(">%d", idxs[i]);
#endif
        if (!idxs_skip[i][idxs[i] - 1]) // if transition will not be skipped
                                        // exit
          return false;
      } else {
        idxs[i] = 0;
 ...
 * @param accum_label a boolean array describing the members of the accumulated
 * label
 * @param current_label a boolean array describing the current transition label
 * @param alphabet_overlap a boolean array describing which elements should be
...
int32_t automaton_automata_check_overlap(signal_bit_array_t *accum_label, signal_bit_array_t *current_label,
  ...
  for (i = 0; i < FIXED_SIGNALS_COUNT; i++) {
    if (accum_label[i] > 0 || current_label[i] > 0)
      break;
    if (i == (FIXED_SIGNALS_COUNT - 1))
      return 0;
  }
  ...
  if (no_overlapping)
    return -1;
  if (at_least_one_does_not)
    return 1;
  if (at_least_one_overlaps)
    return 0;
  return -1;
}
/*
...
(uint32_t *tmp_alphabet, automaton_transition *t, uint32_t alphabet_count)
bool automaton_automata_compose_increment_idxs(uint32_t *idxs, uint32_t
*idxs_sizes, uint32_t automata_count)
 ...
 * @param synch_type the array of synchronization types to be applied
 * sequentially and pairwise for the provided automata
 * @param automata_count the number of automata to be composed
 * @param is_game true if the resulting automaton will hold valuations on its
 * states, false otherwise
 * @param no_mixed_states true if the resulting automaton will have no mixed
 * states (controllable transitions from non controllable states will be
 * removed)
 * @param composition_name the name of the composition as specified in the
 * syntax
...
automaton_automaton *automaton_automata_compose(automaton_automaton **automata, automaton_synchronization_type *synch_type,
  ...
  uint32_t alphabet_count, fluents_count;
  uint32_t *alphabet;
  uint32_t **complementary_alphabet;
  uint32_t *complementary_alphabet_count;
  alphabet_count = 0;
  ...
  for (i = 0; i < automata_count; i++) {
    ...
    for (j = 0; j < alphabet_count; j++) {
      if (TEST_SIGNAL_ARRAY_BIT(boolean_alphabet[i], j))
        printf("%s ", ctx->global_alphabet->list[j].name);
    }
    printf("\n\t[C] ACcumulated boolean alphabet:");
    for (j = 0; j < alphabet_count; j++) {
      if (TEST_SIGNAL_ARRAY_BIT(accumulated_boolean_alphabet[i], j))
        printf("%s ", ctx->global_alphabet->list[j].name);
    }
    printf("\n\t[O] Accumulated boolean alphabet Overlap 1..i:");
    for (j = 0; j < alphabet_count; j++) {
      if (TEST_SIGNAL_ARRAY_BIT(accumulated_alphabet_overlap[i], j))
        printf("%s ", ctx->global_alphabet->list[j].name);
    }
    printf("\n");
  }
  ...
  bool overlaps;
  bool shared_equals;
  // uint32_t* signals_union			= malloc(sizeof(uint32_t) *
  // max_degree_sum); uint32_t signals_union_count	= 0;
  ...
  long int found_misses = 0; // set initial state
  for (i = 0; i < automata_count; i++) {
  ...
  if (is_game) {
    ...
    // maps contiguous index to the index in the automata composition
    vstates_automata_indexes = calloc(vstates_automata_count, sizeof(uint32_t));
    vstates_automata_count = 0;
    // initialize indexes while updating declared state valuations accumulator
    for (i = 0; i < automata_count; i++) {
      if (automata[i]->state_valuations_declared_size > 0) {
  ...
  while (frontier_count > 0) {
    ...
    for (i = 0; i < automata_count; i++) {
      ...
      for (j = 0; j < FIXED_SIGNALS_COUNT; j++)
        automata_accum[i][j] = (signal_bit_array_t)0x0;
    ...
    for (i = 0; i < automata_count; i++) {
      ...
      for (j = 0; j < ctx->global_alphabet->count; j++) {
        if (TEST_SIGNAL_ARRAY_BIT(automata_accum[i], j))
          printf("%s ", ctx->global_alphabet->list[j].name);
      }
      printf("\n");
    }
...
#endif
    for (i = 0; i < FIXED_SIGNALS_COUNT; i++)
      label_accum[i] = (signal_bit_array_t)0x0;
    ...
    while (!automaton_automata_idxs_is_max(idxs, automata_count)) {
      ...
      if (idxs[0] > 0) {
        ...
        for (j = 0; j < FIXED_SIGNALS_COUNT; j++)
          label_accum[j] = (signal_bit_array_t)0x0;
      }
      viable = true;
      ...
      for (j = 1; j < automata_count; j++) {
        ...
        if (idxs[j] == 0) { // transition not being considered
          // is_input = is_input ||
          // TRANSITION_IS_INPUT(automata[j]->transitions[current_state[j]]);
          not_considered = true;
          for (k = 0; k < FIXED_SIGNALS_COUNT; k++) {
        ...
        for (k = 0; k < alphabet_count; k++) {
          if (TEST_SIGNAL_ARRAY_BIT(label_accum, k))
            printf("%s ", ctx->global_alphabet->list[k].name);
        }
        printf("]");
        printf("Current label:[");
        for (k = 0; k < alphabet_count; k++) {
          if (TEST_SIGNAL_ARRAY_BIT(current_label, k))
            printf("%s ", ctx->global_alphabet->list[k].name);
        }
        printf("]");
        printf("Accum. alphabet overlap:[");
        for (k = 0; k < alphabet_count; k++) {
          if (TEST_SIGNAL_ARRAY_BIT(accumulated_alphabet_overlap[j], k))
            printf("%s ", ctx->global_alphabet->list[k].name);
        }
        printf("]");
      ...
      if (viable) {
        ...
        // check if transition can be added (if vstates are consistent between
        // components) state is consistent if:
        //   (a_i & mask_{i-1}) == ( a_{i-1} & mask_i)
        // then we update:
        //   mask <- mask | mask_i
        //   a_{i-1} = a_i | a_{i-1}
  ...
  if (vstates_partial_mask != NULL)
    free(vstates_partial_mask);
  if (vstates_automata_indexes != NULL)
    free(vstates_automata_indexes);
  if (current_vstates != NULL)
    free(current_vstates);
#if VERBOSE
  ...
  free(boolean_alphabet);
  free(accumulated_alphabet_overlap);
  free(accumulated_boolean_alphabet);
  free(current_label);
  free(frontier);
  free(composite_frontier);
  free(current_state);
  free(current_to_state);
  free(idxs);
  free(idxs_size);
  free(label_accum);
  free(idxs_skip);
...
void automaton_composite_tree_destroy(automaton_composite_tree *tree) {
  ...
  tree->key_length = 0;
  uint32_t i;
  for (i = 0; i < tree->entries_size_count; i++)
    free(tree->entries_pool[i]);
