...
void *automaton_ptr_bucket_get_entry(automaton_ptr_bucket_list *list, uint32_t key,
                                     automaton_ptr_bucket_list_key_extractor_func extractor) {
  uint32_t index = key % list->count;
  void **bucket = list->buckets[index];
  bool found = false;
 ...
 *************************/
automaton_concrete_bucket_list *automaton_concrete_bucket_list_create(uint32_t count,
