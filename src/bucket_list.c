...
automaton_bucket_list *
automaton_bucket_list_clone(automaton_bucket_list *source) {
...
bool automaton_bucket_remove_entry(automaton_bucket_list *list,
                                   uint32_t entry) {
  if (!automaton_bucket_has_entry(list, entry))
    return false;
...
void automaton_ptr_bucket_list_initialize(automaton_ptr_bucket_list *bucket,
                                          uint32_t count) {
  uint32_t i;
...
void automaton_ptr_bucket_list_copy(automaton_ptr_bucket_list *target,
                                    automaton_ptr_bucket_list *source) {
...
automaton_ptr_bucket_list *
automaton_ptr_bucket_list_clone(automaton_ptr_bucket_list *source) {
...
void automaton_ptr_bucket_list_merge(
    automaton_ptr_bucket_list *list, automaton_ptr_bucket_list *new_elements,
  ...
  for (i = 0; i < new_elements->count; i++) {
    for (j = 0; j < new_elements->bucket_count[i]; j++) {
      automaton_ptr_bucket_add_entry(list, new_elements->buckets[i][j],
                                     extractor(new_elements->buckets[i][j]));
    }
  }
}

void automaton_ptr_bucket_list_intersect(
    automaton_ptr_bucket_list *list, automaton_ptr_bucket_list *new_elements,
    automaton_ptr_bucket_list_key_extractor_func extractor) {
...
bool automaton_ptr_bucket_has_key(
    automaton_ptr_bucket_list *list, uint32_t key,
    automaton_ptr_bucket_list_key_extractor_func extractor) {
...
bool automaton_ptr_bucket_has_entry(automaton_ptr_bucket_list *list,
                                    void *entry, uint32_t key) {
  uint32_t index = key % list->count;
  void **bucket = list->buckets[index];
  bool found = false;
  uint32_t i;
...
void *automaton_ptr_bucket_get_entry(
    automaton_ptr_bucket_list *list, uint32_t key,
    automaton_ptr_bucket_list_key_extractor_func extractor) {
  uint32_t index = key % list->count;
  void **bucket = list->buckets[index];
...
void *automaton_ptr_bucket_pop_entry(automaton_ptr_bucket_list *list) {
  if (!list->has_last_index)
    return NULL;
  void *entry = list->buckets[list->last_added_bucket]
...
bool automaton_ptr_bucket_add_entry(automaton_ptr_bucket_list *list,
                                    void *entry, uint32_t key) {
...
bool automaton_ptr_bucket_remove_entry(automaton_ptr_bucket_list *list,
                                       void *entry, uint32_t key) {
 ...
 *************************/
automaton_concrete_bucket_list *automaton_concrete_bucket_list_create(
    uint32_t count, automaton_ptr_bucket_list_key_extractor_func extractor_func,
    uint32_t sizeof_element) {
  automaton_concrete_bucket_list *bucket =
      malloc(sizeof(automaton_concrete_bucket_list));
  automaton_concrete_bucket_list_initialize(bucket, count, extractor_func,
                                            sizeof_element);
  return bucket;
}

void automaton_concrete_bucket_list_initialize(
    automaton_concrete_bucket_list *bucket, uint32_t count,
...
void automaton_concrete_bucket_list_copy(
    automaton_concrete_bucket_list *target,
    automaton_concrete_bucket_list *source) {
  if (target->count != source->count ||
    ...
    automaton_concrete_bucket_list_initialize(
        target, source->count, source->extractor_func, source->sizeof_element);
  }
  target->sizeof_element = source->sizeof_element;
  uint32_t i, j;
  for (i = 0; i < target->count; i++) {
    if (target->bucket_size[i] < source->bucket_size[i]) {
      free(target->buckets[i]);
      target->buckets[i] =
          malloc(source->sizeof_element * source->bucket_size[i]);
      target->bucket_size[i] = source->bucket_size[i];
    }
    for (j = 0; j < target->bucket_count[i]; j++) {
      memcpy(GET_CONCRETE_BUCKET_LIST_ENTRY(target, i, j),
             GET_CONCRETE_BUCKET_LIST_ENTRY(source, i, j),
             target->sizeof_element);
    }
    target->bucket_count[i] = source->bucket_count[i];
  }
  target->has_last_index = source->has_last_index;
...
automaton_concrete_bucket_list *
automaton_concrete_bucket_list_clone(automaton_concrete_bucket_list *source) {
  automaton_concrete_bucket_list *target =
      malloc(sizeof(automaton_concrete_bucket_list));
  uint32_t i, j;
  target->composite_count = source->composite_count;
  ...
  for (i = 0; i < target->count; i++) {
    ...
    target->bucket_size[i] = source->bucket_size[i];
    target->buckets[i] =
        malloc(target->sizeof_element * target->bucket_size[i]);
    for (j = 0; j < target->bucket_count[i]; j++) {
      memcpy(GET_CONCRETE_BUCKET_LIST_ENTRY(target, i, j),
             GET_CONCRETE_BUCKET_LIST_ENTRY(source, i, j),
             target->sizeof_element);
...
void automaton_concrete_bucket_list_merge(
    automaton_concrete_bucket_list *list,
    automaton_concrete_bucket_list *new_elements) {
  uint32_t i, j;
  for (i = 0; i < new_elements->count; i++) {
    for (j = 0; j < new_elements->bucket_count[i]; j++) {
      automaton_concrete_bucket_add_entry(
          list, GET_CONCRETE_BUCKET_LIST_ENTRY(new_elements, i, j));
    }
  }
}

void automaton_concrete_bucket_list_intersect(
    automaton_concrete_bucket_list *list,
    automaton_concrete_bucket_list *new_elements) {
  uint32_t i, j;
  void *current_entry;
...
bool automaton_concrete_bucket_has_key(automaton_concrete_bucket_list *list,
                                       uint32_t key) {
  uint32_t index = key % list->count;
  bool found = false;
  uint32_t i;
  for (i = 0; i < list->bucket_count[index]; i++) {
    if (list->extractor_func(GET_CONCRETE_BUCKET_LIST_ENTRY(list, index, i)) ==
        key)
      return true;
  }
  return false;
...
bool automaton_concrete_bucket_has_entry(automaton_concrete_bucket_list *list,
                                         void *entry) {
...
void *automaton_concrete_bucket_get_entry(automaton_concrete_bucket_list *list,
                                          uint32_t key) {
  uint32_t index = key % list->count;
  bool found = false;
...
void automaton_concrete_bucket_pop_entry(automaton_concrete_bucket_list *list,
                                         void *target) {
  if (!list->has_last_index)
    return;
  void *entry = GET_CONCRETE_BUCKET_LIST_ENTRY(
      list, list->last_added_bucket,
      --(list->bucket_count[list->last_added_bucket]));
  list->composite_count--;
...
bool automaton_concrete_bucket_add_entry(automaton_concrete_bucket_list *list,
                                         void *entry) {
  ...
  if (bucket == NULL) {
    list->bucket_size[index] = 2;
    list->buckets[index] =
        calloc(list->bucket_size[index], list->sizeof_element);
    bucket = list->buckets[index];
  }
  ...
  if (list->bucket_count[index] >= (list->bucket_size[index] - 1)) {
    ...
    memcpy(new_bucket, bucket,
           list->sizeof_element * list->bucket_count[index]);
    /*
    for(i = 0; i < list->bucket_count[index]; i++){
            memcpy(GET_CONCRETE_BUCKET_SINGLE_ENTRY(list, new_bucket, i),
    GET_CONCRETE_BUCKET_LIST_ENTRY(list, index, i), list->sizeof_element);
    }
    */
    list->bucket_size[index] = new_size;
    free(bucket);
  ...
  list->has_last_index = true;
  list->last_added_bucket = index;
  memcpy(GET_CONCRETE_BUCKET_LIST_ENTRY(list, index,
                                        (list->bucket_count[index])++),
         entry, list->sizeof_element);
  return false;
}

bool automaton_concrete_bucket_remove_entry(
    automaton_concrete_bucket_list *list, void *entry) {
  if (!automaton_concrete_bucket_has_entry(list, entry))
    return true;
  ...
  for (i = j; i < (int32_t)(list->bucket_count[index] - 1); i++) {
    memcpy(GET_CONCRETE_BUCKET_LIST_ENTRY(list, index, i),
           GET_CONCRETE_BUCKET_LIST_ENTRY(list, index, i + 1),
           list->sizeof_element);
  }
  list->bucket_count[index]--;
