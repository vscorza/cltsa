...
#define GET_MAX_HEAP_ARRAY_ENTRY(heap, array, index)
((void*)(((char*)array) + (index * heap->sizeof_element))) #define
GET_MAX_HEAP_ENTRY(heap, index)	((void*)(((char*)heap->values) + (index *
heap->sizeof_element))) #define GET_MAX_HEAP_L_CHILD(heap, x)
GET_MAX_HEAP_ENTRY(heap,MAX_HEAP_L_INDEX(x)) #define GET_MAX_HEAP_R_CHILD(heap,
x) GET_MAX_HEAP_ENTRY(heap,MAX_HEAP_R_INDEX(x)) #define
...
/*
   ...
   violated In case of deletion of a node, or creating a max heap from an array,
...
heap->cmp_func(GET_MAX_HEAP_L_CHILD(heap, i),GET_MAX_HEAP_ENTRY(heap, i)) > 0) ?
MAX_HEAP_L_INDEX(i) : i; if(MAX_HEAP_R_INDEX(i) < heap->count &&
heap->cmp_func(GET_MAX_HEAP_R_CHILD(heap, i),GET_MAX_HEAP_ENTRY(heap, largest))
> 0) largest = MAX_HEAP_R_INDEX(i); if(largest != i){
                automaton_max_heap_swap(heap, GET_MAX_HEAP_ENTRY(heap, i),
GET_MAX_HEAP_ENTRY(heap, largest)); automaton_max_heap_heapify(heap, largest);
        }
}
*/
void automaton_max_heap_heapify(automaton_max_heap *heap, uint32_t i) {
...
/*
    Function to insert a node into the max heap, by allocating space for that
...
void* automaton_max_heap_add_entry(automaton_max_heap* heap, void* entry) {
        ...
        if(heap->count == heap->size){
                ...
                void* new_values	= malloc(heap->sizeof_element *
new_size); for(i = 0; i < heap->count; i++)
                        memcpy(GET_MAX_HEAP_ARRAY_ENTRY(heap, new_values,
(uint32_t)i), GET_MAX_HEAP_ENTRY(heap, (uint32_t)i), heap->sizeof_element);
                free(heap->values);
    ...
    while(i && heap->cmp_func(entry, GET_MAX_HEAP_PARENT(heap, i)) > 0) {
        automaton_max_heap_swap(heap, GET_MAX_HEAP_ENTRY(heap, i),
GET_MAX_HEAP_PARENT(heap, i)); i = MAX_HEAP_PARENT_INDEX(i) ;
    }
...
void *automaton_max_heap_add_entry(automaton_max_heap *heap, void *entry) {
  ...
  if (heap->count == heap->size) {
    ...
    if (new_values == NULL) {
      printf("Could not allocate memory[automaton_max_heap_add_entry:1]");
      exit(-1);
    }
    heap->values = new_values;
