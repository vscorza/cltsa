...
void automaton_automaton_serialize_metrics(FILE *f, char* filename, automaton_automaton *automaton, bool is_html){
	...
	for(i = 0; i < automaton->transitions_count; i++)if(automaton->out_degree[i] > 0)effective_composite_count += automaton->out_degree[i];
	...
	if(!is_html){
		fprintf(f, "%s,%s,%" PRIu64 ",%" PRIu64 ",%f,%f,%" PRIu64 ",%f,%f,%" PRIu64 ",%f,%f\n%d,", filename, automaton->name, effective_state_count, effective_composite_count,
