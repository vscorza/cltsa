...
void automaton_automaton_print(automaton_automaton *current_automaton, bool print_ctx, bool print_alphabet, bool print_valuations,
  ...
  if (print_alphabet) {
    ...
    for (i = 0; i < current_automaton->local_alphabet_count; i++) {
      printf("%s[%d]%s", ctx->global_alphabet->list[current_automaton->local_alphabet[i]].name, current_automaton->local_alphabet[i],
...
bool automaton_automaton_print_fsp(automaton_automaton *current_automaton, char *filename) {
  ...
  fprintf(f,
          ...
          current_automaton->name, current_automaton->transitions_count, current_automaton->transitions_composite_count,
          controllable_transitions);
  automaton_automata_context *ctx = current_automaton->context;
...
void automaton_fluent_serialize_report(FILE *f, automaton_fluent *fluent) {
  fprintf(f, "%s%s%s%d%s%s", AUT_SER_OBJ_START, fluent->name, AUT_SER_SEP, fluent->starting_signals_count, AUT_SER_SEP,
          AUT_SER_ARRAY_START);
  uint32_t i, j;
  for (i = 0; i < fluent->starting_signals_count; i++) {
...
void automaton_transition_serialize_report(FILE *f, automaton_transition *transition) {
  ...
  fprintf(f, "%s%d%s%d%s%d%s%s", AUT_SER_OBJ_START, transition->state_from, AUT_SER_SEP, transition->state_to, AUT_SER_SEP, signal_count,
...
void automaton_automaton_serialize_metrics(FILE *f, char *filename, automaton_automaton *automaton, bool is_html) {
  ...
  for (i = 0; i < automaton->transitions_count; i++)
    variance_delta_s += (automaton->out_degree[i] - mean_delta_s) * (automaton->out_degree[i] - mean_delta_s);
  ...
  for (i = 0; i < automaton->transitions_count; i++) {
    ...
    for (j = 0; j < automaton->out_degree[i]; j++) {
      if (j > 0) {
        new_monitored =
            !(automaton_automaton_transition_monitored_eq(automaton, &(automaton->transitions[i][j - 1]), &(automaton->transitions[i][j])));
      } else
  ...
  for (i = 0; i < automaton->transitions_count; i++) {
    ...
    variance_controllable_options +=
        (current_controllable - mean_controllable_options) * (current_controllable - mean_controllable_options);
  }
  variance_controllable_options /= ((effective_state_count - 1) * 1.0f);

  if (!is_html) {
    fprintf(f, "%s,%s,%" PRIu64 ",%" PRIu64 ",%f,%f,%" PRIu64 ",%f,%f,%" PRIu64 ",%f,%f\n%d,", filename, automaton->name,
            effective_state_count, effective_composite_count, mean_delta_s, variance_delta_s, total_signals, mean_signals_t,
            variance_signals_t, controllable_options, mean_controllable_options, variance_controllable_options, alphabet_count);
    for (i = 0; i < alphabet_count; i++) {
      fprintf(f, "%s:%" PRIu64 ":%s%s", automaton->context->global_alphabet->list[i].type == INPUT_SIG ? "U" : "C", signal_occurrence[i],
              automaton->context->global_alphabet->list[i].name, i < (alphabet_count - 1) ? "," : "\n");
    }
  } else {
    fprintf(f,
    ...
    fprintf(f,
            "<tr><td>%" PRIu64 "</td><td>%" PRIu64 "</td><td>%f</td><td>%f</td><td>%" PRIu64 "</td><td>%f</td><td>%f</td><td>%" PRIu64
            "</td><td>%f</td><td>%f</td></tr>\n</table>\n",
            effective_state_count, effective_composite_count, mean_delta_s, variance_delta_s, total_signals, mean_signals_t,
            variance_signals_t, controllable_options, mean_controllable_options, variance_controllable_options);
    ...
    for (i = 0; i < alphabet_count; i++) {
      fprintf(f, "<tr><td>%s</td><td>%s</td><td>%" PRIu64 "</td></tr>\n", automaton->context->global_alphabet->list[i].name,
              automaton->context->global_alphabet->list[i].type == INPUT_SIG ? "" : "[*]", signal_occurrence[i]);
    }
    fprintf(f, "</table>\n</body>\n</html>");
  }
...
void automaton_automaton_serialize_report(FILE *f, automaton_automaton *automaton) {
  ...
  for (i = 0; i < automaton->context->state_valuations_count; i++) {
    if (automaton->state_valuations_declared_size == 0) {
      ...
      fprintf(f, "%s%s", TEST_FLUENT_BIT(automaton->state_valuations_declared, fluent_index) ? "1" : "0",
              i == (automaton->context->state_valuations_count - 1) ? "" : AUT_SER_SEP);
    }
  }
  ...
  for (i = 0; i < automaton->transitions_count; i++) {
    ...
    for (j = 0; j < liveness_count; j++) {
      if (automaton->source_type & SOURCE_GAME) {
        ...
        fprintf(f, "%s%s", TEST_FLUENT_BIT(automaton->liveness_valuations, fluent_index) ? "1" : "0",
  ...
  for (i = 0; i < automaton->transitions_count; i++) {
    ...
    for (j = 0; j < automaton->context->state_valuations_count; j++) {
      if ((automaton->source_type & SOURCE_GAME) && automaton->state_valuations_declared_size > 0) {
        ...
        fprintf(f, "%s%s", TEST_FLUENT_BIT(automaton->state_valuations, fluent_index) ? "1" : "0",
                j == (automaton->context->state_valuations_count - 1) ? "" : AUT_SER_SEP);
      } else {
...
void automaton_ranking_transition_serialize_report(FILE *f, automaton_transition *transition, uint32_t ranking_value) {
  fprintf(f, "%s%d%s%d%s%d%s%s", AUT_SER_OBJ_START, transition->state_from, AUT_SER_SEP, transition->state_to, AUT_SER_SEP,
          1 /*transition->signals_count + 1*/, AUT_SER_SEP, AUT_SER_ARRAY_START);
  uint32_t i, k;
  bool first_print = true;
  for (k = 0; k < (TRANSITION_ENTRY_SIZE * FIXED_SIGNALS_COUNT) - 1; k++) {
    if (TEST_TRANSITION_BIT(transition, k)) {
      ...
      for (j = 0; j < automaton->out_degree[i]; j++) {
        if (&(automaton->transitions[i][j]) != NULL) {
          if (first_transition)
            ...
            fprintf(f, "%s", AUT_SER_SEP);
          ranking_value =
              ((automaton_ranking *)automaton_concrete_bucket_get_entry(ranking_list[r], automaton->transitions[i][j].state_to))->value;
          automaton_ranking_transition_serialize_report(f, &(automaton->transitions[i][j]),
                                                        ranking_value == RANKING_INFINITY
                                                            ...
                                                            : automaton->context->global_alphabet->count + ranking_value + 1);
        } else {
          printf("ERROR: Transition was null at [%d][%d]\n", i, j);
        }
        current_count++;
