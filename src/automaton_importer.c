...
void *automaton_automaton_load_array(
  ...
  switch (type) {
  case IMPORT_BOOL:
    sizeof_element = sizeof(bool);
    break;
  case IMPORT_INT:
    sizeof_element = sizeof(uint32_t);
    break;
  case IMPORT_STRING:
  ...
  do {
    ...
    = (bool)v;break; case IMPORT_INT:*((uint32_t *)(&arr + sizeof_element *
    arr_count++))	= (uint32_t)v;break; case IMPORT_STRING:*((char**)(&arr
    + sizeof_element * arr_count++))	= (char*)v;break; default:
                    printf("Import type not defined\n");
                    exit(-1);
                    break;
    }*/
  ...
  do {
    ...
    case IMPORT_BOOL:
      *(bool **)(&arr + sizeof_element * (arr_count++)) = (bool *)v;
      break;
    case IMPORT_INT:
      *(uint32_t **)(&arr + sizeof_element * (arr_count++)) = (uint32_t *)v;
      break;
    case IMPORT_STRING:
      *(char ***)(&arr + sizeof_element * (arr_count++)) = (char **)v;
      break;
    default:
      printf("Import type not defined\n");
      exit(-1);
...
automaton_automaton_load_signal_event(FILE *f, char **buf, uint32_t *buf_size,
  ...
  *last_finalizer = '\0';
  ...
  case 2:
    ...
    break;
  default:
    printf("Corrupt signal event type");
    exit(-1);
    break;
  }
...
void automaton_automaton_check_load_context(FILE *f,
  ...
  if (liveness_names != NULL)
    free(liveness_names);
  if (fluents != NULL)
    free(fluents);
  if (vstates_names != NULL)
    free(vstates_names);
}
