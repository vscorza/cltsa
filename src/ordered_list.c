...
automaton_ordered_list *automaton_ordered_list_clone(automaton_ordered_list *source) {
  ...
  for (i = 0; i < target->count; i++)
    memcpy(GET_ORDERED_LIST_SINGLE_ENTRY(target, target->values, i), GET_ORDERED_LIST_SINGLE_ENTRY(source, source->values, i),
  ...
  for (i = 0; i < target->temporary_count; i++)
    memcpy(GET_ORDERED_LIST_SINGLE_ENTRY(target, target->temporary, i), GET_ORDERED_LIST_SINGLE_ENTRY(source, source->temporary, i),
           target->sizeof_element);
  return target;
}

void automaton_ordered_list_copy(automaton_ordered_list *target, automaton_ordered_list *source) {
  if (target->count != source->count || target->sizeof_element != source->sizeof_element ||
      target->extractor_func != source->extractor_func || target->temporary_size != source->temporary_size) {
...
bool automaton_ordered_list_add_entry(automaton_ordered_list *list, void *entry) {
    ...
    if (list->values == NULL) { // if values list is not initialized copy directly
      ...
      for (i = 0; i < list->temporary_count; i++) {
        memcpy(GET_ORDERED_LIST_SINGLE_ENTRY(list, new_values, (uint32_t)i),
               GET_ORDERED_LIST_SINGLE_ENTRY(list, list->temporary, (uint32_t)i), list->sizeof_element);
#if DEBUG_ORDERED_LIST
      ...
      for (i = 0; i < (int32_t)(list->temporary_count + list->count); i++) {
        ...
        if (temporary_index >= list->temporary_count ||
            (values_index < (list->count) && list->extractor_func(GET_ORDERED_LIST_SINGLE_ENTRY(list, list->temporary, temporary_index)) >
                                                 list->extractor_func(GET_ORDERED_LIST_SINGLE_ENTRY(list, list->values, values_index)))) {
          memcpy(GET_ORDERED_LIST_SINGLE_ENTRY(list, new_values, (uint32_t)i),
                 GET_ORDERED_LIST_SINGLE_ENTRY(list, list->values, values_index++), list->sizeof_element);
#if DEBUG_ORDERED_LIST
          printf("%d(%d),", list->extractor_func(GET_ORDERED_LIST_SINGLE_ENTRY(list, new_values, (uint32_t)i)), values_index - 1);
#endif
        } else {
          memcpy(GET_ORDERED_LIST_SINGLE_ENTRY(list, new_values, (uint32_t)i),
                 GET_ORDERED_LIST_SINGLE_ENTRY(list, list->temporary, temporary_index++), list->sizeof_element);
#if DEBUG_ORDERED_LIST
  ...
  if (list->temporary_count > 0) {
    for (i = (int32_t)list->temporary_count - 1; i >= 0; i--) {
      if (list->extractor_func(GET_ORDERED_LIST_SINGLE_ENTRY(list, list->temporary, (uint32_t)i)) > list->extractor_func(entry)) {
        memcpy(GET_ORDERED_LIST_SINGLE_ENTRY(list, list->temporary, (uint32_t)(i + 1)),
               GET_ORDERED_LIST_SINGLE_ENTRY(list, list->temporary, (uint32_t)i), list->sizeof_element);
      } else {
        found = true;
