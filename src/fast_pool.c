...
void *automaton_fast_pool_get_instance(automaton_fast_pool *pool, uint32_t *fragment_ID) {
  ...
  if (pool->next_free_element == (uintptr_t)NULL) {
    ...
    uintptr_t *ptr_void = realloc(pool->fragments_last_free_element, new_size * sizeof(uintptr_t));
    if (ptr_void != NULL) {
      pool->fragments_last_free_element = ptr_void;
      pool->fragments_last_free_element[pool->size - 1] = pool->fragments[pool->size - 1];
    } else {
      printf("Could not get new instance from pool\n");
      exit(-1);
    }
  ...
  *fragment_ID = pool->next_free_element_fragment_index;
  ...
  pool->fragments_last_free_element[pool->next_free_element_fragment_index] =
      (pool->fragments_last_free_element[pool->next_free_element_fragment_index] + (uintptr_t)pool->sizeof_element);
  pool->fragments_last_index[pool->next_free_element_fragment_index]++;
  if (pool->fragments_last_index[pool->next_free_element_fragment_index] < (pool->fragment_size - 1)) {
    pool->next_free_element = ((uintptr_t)pool->fragments_last_free_element[pool->next_free_element_fragment_index]);
  } else {
...
bool automaton_fast_pool_release_instance(automaton_fast_pool *pool, uint32_t fragment_ID) {
  ...
  if (pool->fragments_count[fragment_ID] == 0) { // if no element reset last index
    ...
    if (pool->next_free_element_fragment_index == fragment_ID)
      pool->next_free_element = (uintptr_t)pool->fragments_last_free_element[fragment_ID];
  }
#if DEBUG_POOL
