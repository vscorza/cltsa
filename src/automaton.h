...
#define TEST_TRANSITION_BIT(t, index)                                                                                                      \
  ((t->signals[((index + 1) / TRANSITION_ENTRY_SIZE)]) & ((signal_bit_array_t)1 << ((index + 1) % TRANSITION_ENTRY_SIZE)))
#define SET_TRANSITION_BIT(t, index)                                                                                                       \
  ((t->signals[((index + 1) / TRANSITION_ENTRY_SIZE)]) |= ((signal_bit_array_t)1 << ((index + 1) % TRANSITION_ENTRY_SIZE)))
#define SET_TRANSITION_MASK_BIT(m, index)                                                                                                  \
  ((m[((index + 1) / TRANSITION_ENTRY_SIZE)]) |= ((signal_bit_array_t)1 << ((index + 1) % TRANSITION_ENTRY_SIZE)))
#define CLEAR_TRANSITION_BIT(t, index)                                                                                                     \
  ((t->signals[((index + 1) / TRANSITION_ENTRY_SIZE)]) &= ~(((signal_bit_array_t)1 << ((index + 1) % TRANSITION_ENTRY_SIZE))))
#define TEST_SIGNAL_ARRAY_BIT(arr, index)                                                                                                  \
  ((arr[((index + 1) / TRANSITION_ENTRY_SIZE)]) & ((signal_bit_array_t)1 << ((index + 1) % TRANSITION_ENTRY_SIZE)))
...
#define GET_TRANSITION_SIGNAL_COUNT(transition)                                                                                            \
  ...
  for (_p_ = 0; _p_ < (TRANSITION_ENTRY_SIZE * FIXED_SIGNALS_COUNT) - 1; _p_++) {                                                          \
...
void automaton_automaton_print(automaton_automaton *current_automaton, bool print_ctx, bool print_alphabet, bool print_valuations,
...
bool automaton_ranking_print_report(automaton_automaton *automaton, automaton_concrete_bucket_list **ranking_list, uint32_t *max_delta,
...
void automaton_automaton_check_load_context(FILE *f, automaton_automata_context *ctx, char **buf, uint32_t *buf_size, char *last_finalizer);
...
bool automaton_transition_add_signal_event(automaton_transition *transition, automaton_automata_context *ctx,
                                           automaton_signal_event *signal_event);
bool automaton_transition_add_signal_event_ID(automaton_transition *transition, automaton_automata_context *ctx, uint32_t index,
                                              automaton_signal_type type);
/** FLUENT **/
bool automaton_fluent_has_starting_signals(automaton_fluent *fluent, automaton_alphabet *alphabet, uint32_t signal_count,
                                           automaton_signal_event **signal_events);
bool automaton_fluent_add_starting_signals(automaton_fluent *fluent, automaton_alphabet *alphabet, uint32_t signal_count,
                                           automaton_signal_event **signal_events);
bool automaton_fluent_has_ending_signals(automaton_fluent *fluent, automaton_alphabet *alphabet, uint32_t signal_count,
                                         automaton_signal_event **signal_events);
bool automaton_fluent_add_ending_signals(automaton_fluent *fluent, automaton_alphabet *alphabet, uint32_t signal_count,
                                         automaton_signal_event **signal_events);
/** VALUATION **/
...
automaton_automaton *automaton_get_gr1_unrealizable_minimization_dd2(
    ...
    uint32_t guarantees_count, uint8_t *partition_bit_vector, uint32_t transitions_kept_size, uint32_t paritions_count, uint32_t t_count,
