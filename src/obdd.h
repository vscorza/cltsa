...
#define GET_VAR_INDEX(variable_count, valuation_index, variable_index)         \
  ((((variable_count) * (valuation_index) + (variable_index)) * sizeof(bool)))
#define GET_VAR_IN_VALUATION(arr, variable_count, valuation_index,             \
                             variable_index)                                   \
  (arr[GET_VAR_INDEX(variable_count, valuation_index, variable_index)])
...
obdd *obdd_restrict_vector(obdd *root, uint32_t *var_ids, bool *values,
                           uint32_t count);
obdd_node *obdd_node_restrict_vector(obdd_mgr *mgr, obdd_node *root,
                                     uint32_t *var_ids, bool *values,
                                     uint32_t current_index, uint32_t count);
...
obdd *obdd_apply(bool (*apply_fkt)(bool, bool), obdd *left,
                 obdd *right); // if not canonical results should be reduced
...
bool obdd_is_constant(obdd_mgr *mgr,
                      obdd_node *root); // checks if representation is constant
bool obdd_is_tautology(
    obdd_mgr *mgr, obdd_node *root); // checks if representation is always true
bool obdd_is_sat(obdd_mgr *mgr,
                 obdd_node *root); // checks if representation is satisfiable
...
obdd_node *obdd_cache_constant_lookup(obdd_cache *cache, uintptr_t op,
                                      obdd_node *f, obdd_node *g, obdd_node *h);
