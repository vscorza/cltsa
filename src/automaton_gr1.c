...
void automaton_pending_state_destroy(automaton_pending_state *pending_state) { free(pending_state); }
 ...
 * best_j(v)
 */
automaton_ranking *automaton_state_best_successor_ranking(automaton_automaton *game_automaton, uint32_t state,
  ...
  for (i = 0; i < game_automaton->out_degree[state]; i++) {
    if (i > 0)
      new_monitored = !(automaton_automaton_transition_monitored_eq(game_automaton, &(game_automaton->transitions[state][i - 1]),
...
bool automaton_ranking_lt(automaton_ranking *left, automaton_ranking *right) { return automaton_ranking_gt(right, left); }
bool automaton_state_is_stable(automaton_automaton *game_automaton, uint32_t state, automaton_concrete_bucket_list **ranking,
   ...
   */
  uint32_t fluent_count = game_automaton->context->global_fluents_count;
  uint32_t fluent_index = GET_STATE_FLUENT_INDEX(fluent_count, state,
                                                 guarantees_indexes[current_guarantee]); // guarantees_indexes[current_guarantee]);
  bool satisfies_guarantee = TEST_FLUENT_BIT(game_automaton->valuations, fluent_index);
  /**
   * v in P_i
   */
   ...
   */
  j = current_guarantee;
  automaton_ranking *sr = automaton_state_best_successor_ranking(game_automaton, state, ranking, j, guarantee_count, guarantees_indexes);
...
void automaton_add_unstable_predecessors(automaton_automaton *game_automaton, automaton_pending_state_max_heap *pending_list,
                                         automaton_concrete_bucket_list *key_list, uint32_t state, automaton_concrete_bucket_list **ranking,
                                         uint32_t current_guarantee, uint32_t guarantee_count, uint32_t assumptions_count,
                                         uint32_t *guarantees_indexes, uint32_t *assumptions_indexes, int32_t first_assumption_index,
                                         uint32_t timestamp) {
  uint32_t i;
  ...
  for (i = 0; i < game_automaton->in_degree[state]; i++) {
    ...
    // value
    current_ranking =
        ((automaton_ranking *)automaton_concrete_bucket_get_entry(ranking[current_guarantee], current_transition->state_from));
    if (current_ranking == NULL)
      continue;
...
void automaton_ranking_increment(automaton_automaton *game_automaton, automaton_concrete_bucket_list **ranking,
                                 automaton_ranking *current_ranking, uint32_t ref_state, uint32_t *max_delta, uint32_t current_guarantee,
                                 uint32_t guarantee_count, uint32_t assumptions_count, uint32_t *guarantees_indexes,
                                 uint32_t *assumptions_indexes, uint32_t first_assumption_index, automaton_ranking *target_ranking) {
...
void automaton_ranking_update(automaton_automaton *game_automaton, automaton_concrete_bucket_list **ranking,
                              automaton_ranking *current_ranking, uint32_t *max_delta, uint32_t current_guarantee, uint32_t guarantee_count,
                              uint32_t assumptions_count, uint32_t *guarantees_indexes, uint32_t *assumptions_indexes,
                              uint32_t first_assumption_index) {
  automaton_ranking incr_ranking;
  /**
   * get best_j(v)
   */
  automaton_ranking *best_ranking = automaton_state_best_successor_ranking(game_automaton, current_ranking->state, ranking,
                                                                           current_guarantee, guarantee_count, guarantees_indexes);
#if DEBUG_SYNTHESIS
  bool satisfies_guarantee = false;
  if (_print_ranking) {
    uint32_t fluent_index = GET_STATE_FLUENT_INDEX(game_automaton->context->global_fluents_count, current_ranking->state,
   ...
   */
  automaton_ranking_increment(game_automaton, ranking, best_ranking, current_ranking->state, max_delta, current_guarantee, guarantee_count,
                              assumptions_count, guarantees_indexes, assumptions_indexes, first_assumption_index, &incr_ranking);
...
#if DEBUG_SYNTHESIS
  printf("\t[U]<%d,%d,(%d,%d)->(%d,%d)> <%d,(%d,%d)>%s", incr_ranking.state, current_guarantee, old_value, old_ass, incr_ranking.value,
         incr_ranking.assumption_to_satisfy, best_ranking->state, best_ranking->value, best_ranking->assumption_to_satisfy,
...
uint32_t *automaton_compute_infinity(automaton_automaton *game_automaton, uint32_t assumptions_count, uint32_t guarantees_count,
...
#if DEBUG_SYNTHESIS
  printf("<a id='synthesis'>Starting synthesis</a>[<a href='#synthesis'>Synthesis "
...
void automaton_get_gr1_liveness_indexes(automaton_automaton *game_automaton, char **assumptions, uint32_t assumptions_count,
                                        char **guarantees, uint32_t guarantees_count, uint32_t **assumptions_indexes,
...
automaton_automaton *automaton_get_gr1_strategy(automaton_automaton *game_automaton, char **assumptions, uint32_t assumptions_count,
  ...
  for (i = 0; i < guarantees_count; i++)
    key_lists[i] =
        automaton_concrete_bucket_list_create(RANKING_BUCKET_SIZE, automaton_pending_state_extractor, sizeof(automaton_pending_state));
  ...
  automaton_get_gr1_liveness_indexes(game_automaton, assumptions, assumptions_count, guarantees, guarantees_count, &assumptions_indexes,
                                     &guarantees_indexes);
  ...
  uint32_t *max_delta =
      automaton_compute_infinity(game_automaton, assumptions_count, guarantees_count, assumptions_indexes, guarantees_indexes);
        ...
        if (assumptions_count == 0 || (TEST_FLUENT_BIT(game_automaton->valuations, fluent_index))) {
          for (l = 0; l < guarantees_count; l++) {
            ...
            if (!TEST_FLUENT_BIT(game_automaton->valuations, fluent_index)) {
              ...
              concrete_pending_state.goal_to_satisfy = j; // TODO: check this, was ...goal_to_satisfy =
...
#endif
        automaton_add_unstable_predecessors(game_automaton, pending_list, key_lists[j], i, ranking_list,
                                            /*guarantees_indexes[j] <- WAS*/ j, guarantees_count, assumptions_count, guarantees_indexes,
                                            assumptions_indexes, assumptions_count > 0 ? assumptions_indexes[first_assumption_index] : NULL,
                                            pending_processed);
      }
    }
  }
  ...
  while (pending_list->count > 0) {
    ...
    state_location =
        (automaton_pending_state *)automaton_concrete_bucket_get_entry(key_lists[goal_to_satisfy], current_pending_state.state);
    ...
    current_ranking = (automaton_ranking *)automaton_concrete_bucket_get_entry(ranking_list[current_pending_state.goal_to_satisfy],
                                                                               current_pending_state.state);
#if DEBUG_SYNTHESIS
    if (_print_ranking)
      printf("\t[<-]<%d,%d,(%d,%d)>%s", current_pending_state.state, current_pending_state.goal_to_satisfy, current_ranking->value,
             current_ranking->assumption_to_satisfy, (__count_updates_gr1++ % __count_upgrades_gr1_tabs) == 0 ? "\n" : "");
#endif
    if (current_ranking->value == RANKING_INFINITY) {
      pending_processed++;
      continue;
    ...
    if (automaton_state_is_stable(game_automaton, current_pending_state.state, ranking_list, current_pending_state.goal_to_satisfy,
                                  guarantees_count, assumptions_count, guarantees_indexes, assumptions_indexes,
    ...
    automaton_ranking_update(game_automaton, ranking_list,
                             ...
                             // current_pending_state.state)
                             ,
                             max_delta, current_pending_state.goal_to_satisfy, guarantees_count, assumptions_count, guarantees_indexes,
                             assumptions_indexes, first_assumption_index);
#if DEBUG_SYNTHESIS
    if (_print_ranking)
      printf("\t[->]<%d,%d,(%d, %d)>%s", current_pending_state.state, current_pending_state.goal_to_satisfy, current_ranking->value,
             current_ranking->assumption_to_satisfy, (__count_updates_gr1++ % __count_upgrades_gr1_tabs) == 0 ? "\n" : "");
#endif
    automaton_add_unstable_predecessors(game_automaton, pending_list, key_lists[goal_to_satisfy], current_pending_state.state, ranking_list,
                                        current_pending_state.goal_to_satisfy, guarantees_count, assumptions_count, guarantees_indexes,
                                        assumptions_indexes, first_assumption_index, pending_processed); //**21
    ...
    if (!ranking_was_infinity && current_ranking->value == RANKING_INFINITY) {
      for (i = 0; i < guarantees_count; i++) {
        if (i != current_pending_state.goal_to_satisfy) {
          ...
          automaton_add_unstable_predecessors(game_automaton, pending_list, key_lists[i], current_pending_state.state, ranking_list, i,
                                              guarantees_count, assumptions_count, guarantees_indexes, assumptions_indexes,
  ...
  if (_print_ranking) {
    ...
    for (i = 0; i < game_automaton->transitions_count; i++) {
      ...
      printf("[%s][<a id='rank_%d_%d'>S %d</a>:", game_automaton->is_controllable[i] ? "C" : "U", __ranking_link_id, i, i);
      for (j = 0; j < guarantees_count; j++) {
  ...
  // build strategy
  strategy = automaton_automaton_create(strategy_name, game_automaton->context, game_automaton->local_alphabet_count,
                                        game_automaton->local_alphabet, false, false, true, false);

  bool is_winning = true;
  for (i = 0; i < guarantees_count; i++) {
    automaton_ranking *initial_ranking =
        ((automaton_ranking *)automaton_concrete_bucket_get_entry(ranking_list[i], game_automaton->initial_states[0]));
            ...
            for (l = 0; l <= game_automaton->out_degree[current_ranking->state]; l++) {
              ...
              if (new_monitored || (l == game_automaton->out_degree[current_ranking->state])) {
                if (min_ranking == NULL) {
                  current_transition =
                      &(game_automaton
                            ->transitions[current_ranking->state][(l == game_automaton->out_degree[current_ranking->state]) ? (l - 1) : l]);
                  succ_ranking =
                      (automaton_ranking *)automaton_concrete_bucket_get_entry(ranking_list[succ_guarantee], current_transition->state_to);
                  min_value = succ_ranking->value;
                  min_ranking = succ_ranking;
                  min_transition = current_transition;
                } else {
                  succ_ranking =
                      (automaton_ranking *)automaton_concrete_bucket_get_entry(ranking_list[succ_guarantee], min_transition->state_to);
                }
...
bool automaton_is_gr1_realizable(automaton_automaton *game_automaton, char **assumptions, uint32_t assumptions_count, char **guarantees,
  ...
  bool is_realizable = (strategy->transitions_count != 0 && strategy->out_degree[strategy->initial_states[0]] > 0);
